...
public readonly struct ProbabilityThreshold<T> : IComparable<ProbabilityThreshold<T>>, IComparable
{
    ...
    public int CompareTo(ProbabilityThreshold<T> other) => Threshold.CompareTo(other.Threshold);
...
public static class ProbabilityThreshold
{
    ...
    {
        ProbabilityThreshold.ValidateProbabilityThreshold(probability);
    ...
    public static ImmutableArray<ProbabilityThreshold<T>> ToProbabilityThresholds<T>(
        this ImmutableArray<Weighted<T>> weights
    )
    {
        if (weights.Length == 0)
            return [];
