...
public sealed class EventStateTimelineMapCreateTest
{
    ...
    public async Task EmptyEventTimeline_ResultsIn_StateTimelinesOnly()
    {
        ...
        await Assert.That(result)
            ...
            .And.Satisfies(x => x.EventTimeline.ToImmutableArray(), assert => assert.IsEmpty())
            .And.Satisfies(x => x.StateTimelineMap.StateTimelines, assert => assert.IsEquivalentTo(stateTimelines));
    }
    ...
    public async Task LesserDuration_ResultsIn_TrimmedTimelines()
    {
        ...
        await Assert.That(result)
            ...
            .And.Satisfies(x => x.StateTimelineMap.StateTimelines, assert => assert.IsEquivalentTo(expectedStateTimelines));
    }
