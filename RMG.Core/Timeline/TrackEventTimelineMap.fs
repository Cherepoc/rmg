...
module TrackEventTimelineMap =
    ...
    let fromSeq<'T> (input: TrackEventTimelineMapInput<'T>) : TrackEventTimelineMap<'T> =
        ...
        |> Seq.map
            (fun (trackNumber, trackTimeline) ->
                let mergedTimeline =
                    trackTimeline
                    |> Seq.map (fun (_, timeline) -> timeline |> EventTimeline.fromSequence |> Timeline.itemFromSingle)
                    |> EventTimeline.merge
    ...
    let map<'TSource, 'TDest> (func: 'TSource -> 'TDest) (input: TrackEventTimelineMap<'TSource>) : TrackEventTimelineMap<'TDest> =
        input
        |> Map.map (fun _ trackTimeline -> trackTimeline |> Timeline.map func |> EventTimeline.fromSequence)

    let mergeItem (item: 'T) (timelineItemMerge: TimelineItemMerge<'T>) (inputTimelineMap: TrackEventTimelineMap<'T>) : TrackEventTimelineMap<'T> =
        inputTimelineMap |> map (fun x -> timelineItemMerge (x, item))

    let mergeComposite (timelineItemMerge: TimelineItemMerge<'T>) (inputTimelineMap: Timeline<TrackEventTimelineMap<'T> * 'T>) : TrackEventTimelineMap<'T> =
        inputTimelineMap
        |> Timeline.map
