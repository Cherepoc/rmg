...
type CompositeEventPattern<'T>
    ...
    new(duration: Duration, timeline: Timeline<'T>, compositePatternTimeline: Timeline<CompositeEventPattern<'T> * 'T>, timelineItemMerge: TimelineItemMerge<'T>) =
        let orderedTimeline = timeline |> EventTimeline.fromSequence

        let orderedPatternTimeline = compositePatternTimeline |> EventTimeline.fromSequence
