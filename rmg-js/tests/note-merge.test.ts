import { emptyNoteBaseTimeline, NoteBaseTimeline } from '../src/music/note-base';
...
describe('Note timeline merge', () => {
  it('correctly merges', () => {
    ...
    expect(mergeResult).toStrictEqual(<NoteBaseTimeline>{
      key: [
        timelineItem(0, 1),
        timelineItem(1, 2),
        timelineItem(2, 1),
  ...
  it('leaves no duplicates', () => {
    const source = emptyNoteBaseTimeline();
    const target: NoteBaseTimeline = {
      key: [timelineItem(0, 1)],
      octave: [timelineItem(0, 1)],
      volume: [timelineItem(0, 0.5)],
  ...
  it('produces empty result if inputs are empty', () => {
    const source = emptyNoteBaseTimeline();
    const target: NoteBaseTimeline = emptyNoteBaseTimeline();
    const mergeResult = mergeNoteBaseTimelines(source, target, 0, 1);
    expect(mergeResult).toStrictEqual(emptyNoteBaseTimeline());
  });
});
